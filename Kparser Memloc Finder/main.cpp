...
int __cdecl main(int argc, char* argv[])
{
    ...
    auto chatPointer = FindPattern(procDump, procSize, (BYTE*)"\x8B\x0D\xFF\xFF\xFF\xFF\x85\xC9\x74\x0F\x8B\xFF\xFF\xFF\xFF\xFF\x8B", "xx????xxxxx?????x");
    if (chatPointer == NULL)
    {
        ::CloseHandle(handle);
        delete[] procDump;
    ...
    chatPointer -= (DWORD)procDump;
    std::cout << "[*] Found signature for chat!" << std::endl;

    // Read the chat pointer..
    ::ReadProcessMemory(handle, (LPCVOID)(procBase + chatPointer + 2), &chatPointer, 4, NULL);
    chatPointer += 4;
    chatPointer -= procBase;
    std::cout << "[*] Chat Memory Offset: " << std::hex << chatPointer;
